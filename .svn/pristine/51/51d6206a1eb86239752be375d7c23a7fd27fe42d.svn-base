...
double ProofGraph::doReduction(double solving_time)
{
	if(enabledTransfTraversals())
	{
		if(  ( ratioReductionSolvingTime() > 0 && reductionTime() > 0) ||
			...
			periplo_error( "Please set either ratio or time for reduction or number of proof traversals" );
	}
	if ( reductionLoops() == 0 )
		periplo_error( "Please set number of global reduction loops to at least 1" );
	...
	if ( verbose() > 0 )
	{
		...
		if( enabledPushDownUnits() ) cerr << "# preceded by recycle units" << endl;
		cerr << "# Each global iteration consists of: " << endl;
		if( enabledStructuralHashing() ) cerr << "# Structural hashing" << endl;
		if( enabledRecyclePivots() ) cerr << "# Recycle pivots" << endl;
		if( enabledTransfTraversals() )
		{
			cerr << "# Transformation traversals ";
			if( ratioReductionSolvingTime() > 0 || reductionTime() > 0)
