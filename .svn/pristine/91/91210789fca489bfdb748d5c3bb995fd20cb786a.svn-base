...
void ProofGraph::verifyLeavesInconsistency( )
{
	...
	for ( int i = 0 ; i < proofleaves.size( ) ; i ++ )
	{
		...
		solver.printClause( dump_out, getNode(proofleaves[ i ])->getClause(), false );
		dump_out << endl;
	}
	...
	if ( tool_res == true )
		periplo_error( "External tool says the set of proof leaves is satisfiable" );
...
void ProofGraph::checkProof( bool check_clauses )
{
	...
	for(unsigned u = 0; u < getGraphSize(); u ++)
	{
		if( isSetVisited1(u) && !isSetVisited2(u)) {
			cerr << "Node " << u << " is unreachable going top-down" << endl; periplo_error_();}
		if( !isSetVisited1(u) && isSetVisited2(u)) {
			cerr << "Node " << u << " is unreachable going bottom-up" << endl; periplo_error_();}
	}

	// Ensure that there are no useless leaves
	for(set<clauseid_t>::iterator it = leaves_ids.begin(); it != leaves_ids.end(); it++)
	{
		if(!isSetVisited1(*it)) { cerr << "Detached leaf" << (*it) << endl; periplo_error_();}
	}
