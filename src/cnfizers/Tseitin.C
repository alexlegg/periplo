		    ...
		    )
{
  ...
  while( !unprocessed_enodes.empty( ) )
  {
	...
	cnfizeXor( new_arg_list
      ...
      {
	periplo_error2( "operator not handled ", enode->getCar( ) );
      }
...
#ifdef PRODUCE_PROOF
      if ( config.produce_inter > 0 )
	solver.addClause( little_clause, partitions );
      else
#endif
      solver.addClause( little_clause );        // Adds a little clause to the solver
    }
  }
  else
  {
...
#ifdef PRODUCE_PROOF
      if ( config.produce_inter > 0 )
	solver.addClause( big_clause, partitions );
      else
#endif
    solver.addClause( big_clause );                    // Adds a big clause to the solver
  }
}
...
#ifdef PRODUCE_PROOF
    if ( config.produce_inter > 0 )
      solver.addClause( clause, partitions );
    else
#endif
...
#endif
    solver.addClause( clause ); // Adds a little clause to the solver
    clause.pop_back( );
    clause.pop_back( );
